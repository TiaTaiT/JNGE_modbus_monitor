...
def print_header():
    ...
    print("\n" + "=" * 100)
    print("JNGE MPPT Controller - Real-Time Monitoring")
    print("=" * 100)
    print("Press Ctrl+C to stop monitoring")
    print("-" * 100)


def print_monitoring_line(params: Dict, iteration: int):
    """Print formatted monitoring line"""
    # Clear line and move cursor up (ANSI escape codes)
    if iteration > 0:
        print("\033[F" * 8, end="")  # Move up 8 lines
    ...
    # Determine charge status symbol
    charge_symbols = {
        ChargeState.UNCHARGED: "○",
        ChargeState.CONSTANT_CURRENT: "◐",
        ChargeState.BOOST_CHARGING: "◕",
        ChargeState.FLOAT_CHARGING: "●"
    }
    charge_symbol = charge_symbols.get(params['charge_state'], "?")
    
    # Get faults
    fault_list = []
    if params['error_code'] != 0:
        for bit, desc in JNGEController.FAULT_CODES.items():
            if params['error_code'] & (1 << bit):
                fault_list.append(desc)
    
    # Print status
    print(f"\n┌─── BATTERY ────────────────────────────────────────────────────────┐")
    print(f"│ Voltage: {params['battery_voltage']:5.1f} V  │  State: {params['charge_state'].name:20s} {charge_symbol} │")
    print(f"├─── SOLAR (PV) ─────────────────────────────────────────────────────┤")
    print(f"│ Voltage: {params['pv_voltage']:5.1f} V  │  Current: {params['pv_current']:5.1f} A  │  Power: {params['pv_power']:5d} W │")
    print(f"├─── WIND (FAN) ─────────────────────────────────────────────────────┤")
    print(f"│ Voltage: {params['fan_voltage']:5.1f} V  │  Current: {params['fan_current']:5.1f} A  │  Power: {params['fan_power']:5d} W │")
    print(f"├─── TOTALS ─────────────────────────────────────────────────────────┤")
    print(f"│ Power: {total_power:5d} W  │  Energy: {total_energy:7.1f} kWh  │  Load: {'ON ' if params['load_status'] == LoadStatus.BOOT_UP else 'OFF'} │")
    
    if fault_list:
        fault_str = ", ".join(fault_list)
        if len(fault_str) > 60:
            fault_str = fault_str[:57] + "..."
        print(f"├─── FAULTS ─────────────────────────────────────────────────────────┤")
        print(f"│ ⚠  {fault_str:62s} │")
    
    print(f"└────────────────────────────────────────────────────────────────────┘")


def monitor_realtime(controller: JNGEController, interval: float = 2.0):
    ...
    try:
        while True:
            ...
            if params:
                print_monitoring_line(params, iteration)
                iteration += 1
            else:
...
def show_system_info(controller: JNGEController):
    ...
    if device_info:
        device_type = "Wind and Solar Hybrid Controller" if device_info['device_type'] == 0x4100 else "Unknown"
        print(f"\nDevice Type: {device_type} (0x{device_info['device_type']:04X})")
    ...
    if settings:
        ...
        print(f"  Boost Charging: {settings['boost_voltage']:.1f} V")
        print(f"  Float Charging: {settings['float_voltage']:.1f} V")
        print(f"  Low Voltage Disconnect: {settings['undervoltage']:.1f} V")
        print(f"  Recovery Voltage: {settings['undervoltage_recovery']:.1f} V")
    ...
    if params:
        print(f"\nCurrent Status:")
        print(f"  Battery: {params['battery_voltage']:.1f} V")
        print(f"  PV Rating: {params['pv_rating']} W")
        print(f"  Fan Rating: {params['fan_rating']} W")
        print(f"  Total PV Energy: {params['pv_total_energy']:.1f} kWh")
        print(f"  Total Fan Energy: {params['fan_total_energy']:.1f} kWh")
...
def main():
    """Real-time monitoring with system info"""
    ...
    # Disable debug mode for cleaner monitoring output
    controller.set_debug(False)
    
    # Connect
    print("Connecting to JNGE Controller...")
    ...
    try:
        # Test communication
        test_values = controller.read_registers(0x1000, 0x01)
        if not test_values:
        ...
        monitor_realtime(controller, interval=2.0)
